...
def sample_images(batches_done):
    ...
    key = torch.load('C:/Users/XXX/PycharmProjects/secStudent/saved_key/hinet_key.pth')
    key = key.unsqueeze(0)
    ...
    with autocast():
        for i in tqdm(range(len(images_name_lists))):
            ...
            # save_image(real_A, CUHK_light_op_path + '/' + pic_name[0]+'real_A'+'.png', normalize=False)
            # save_image(fake_B, CUHK_light_op_path + '/' + pic_name[0]+'fake_B'+'.png', normalize=False)
            # save_image(cycle_A, CUHK_light_op_path+'/'+ pic_name[0]+'cycle_A'+'.png', normalize=False)
            # Arange images along y-axis
            image_grid = torch.cat((real_A, fake_B, cycle_A,), 1)
